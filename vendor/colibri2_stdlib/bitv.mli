...
(** [(Bitv.create n b)] creates a new bit vector of length [n], initialized with
    [b]. *)

val init : int -> (int -> bool) -> t
(** [(Bitv.init n f)] returns a fresh vector of length [n], with bit number [i]
    initialized to the result of [(f i)]. *)

val random : int -> t
(** [Bitv.random n] returns a fresh vector of length [n] with random bits. This
    is equivalent to [Bitv.init n (fun _ -> Random.bool ())], but much faster.
*)
...
(** [Bitv.tanimoto v1 v2] is |inter(v1,v2)| / |union(v1,v2)|. Also called the
    Jaccard score. (1 - tanimoto) is a proper distance. raises
    [Invalid_argument] if the two vectors do not have the same length *)

val max_length : int
(** This is typically [Sys.max_string_length * 8] but may be smaller on small
...
val copy : t -> t
(** [(Bitv.copy v)] returns a copy of [v], that is, a fresh vector containing
    the same elements as [v]. *)

val append : t -> t -> t
(** [(Bitv.append v1 v2)] returns a fresh vector containing the concatenation of
...
val sub : t -> int -> int -> t
(** [(Bitv.sub v start len)] returns a fresh vector of length [len], containing
    the bits number [start] to [start + len - 1] of vector [v]. Raise
    [Invalid_argument "Bitv.sub"] if [start] and [len] do not designate a valid
    subvector of [v]; that is, if [start < 0], or [len < 0], or
    [start + len > Bitv.length a]. *)

val fill : t -> int -> int -> bool -> unit
(** [(Bitv.fill v ofs len b)] modifies the vector [v] in place, storing [b] in
    elements number [ofs] to [ofs + len - 1]. Raise
    [Invalid_argument "Bitv.fill"] if [ofs] and [len] do not designate a valid
    subvector of [v]. *)

val blit : t -> int -> t -> int -> int -> unit
(** [(Bitv.blit v1 o1 v2 o2 len)] copies [len] elements from vector [v1],
    starting at element number [o1], to vector [v2], starting at element number
    [o2]. It does not work correctly if [v1] and [v2] are the same vector with
    the source and destination chunks overlapping. Raise
    [Invalid_argument "Bitv.blit"] if [o1] and [len] do not designate a valid
    subvector of [v1], or if [o2] and [len] do not designate a valid subvector
    of [v2]. *)
...
(** Given a function [f], [(Bitv.map f v)] applies [f] to all the elements of
    [v], and builds a vector with the results returned by [f]. *)

val iteri : (int -> bool -> unit) -> t -> unit

val mapi : (int -> bool -> bool) -> t -> t
(** [Bitv.iteri] and [Bitv.mapi] are similar to [Bitv.iter] and [Bitv.map]
    respectively, but the function is applied to the index of the element as
    first argument, and the element itself as second argument. *)

val fold_left : ('a -> bool -> 'a) -> 'a -> t -> 'a
(** [(Bitv.fold_left f x v)] computes
    [f (... (f (f x (get v 0)) (get v 1)) ...) (get v (n-1))], where [n] is the
    length of the vector [v]. *)

val fold_right : (bool -> 'a -> 'a) -> t -> 'a -> 'a
(** [(Bitv.fold_right f a x)] computes
    [f (get v 0) (f (get v 1) ( ... (f (get v (n-1)) x) ...))], where [n] is the
    length of the vector [v]. *)
...
val pop_and : t -> t -> int
(** Count of the population of common bits, i.e., number of 1 bits *)

val xor_and : t -> t -> bool
(** Parity of the population of common bits, i.e., odd number of 1 bits *)

val iteri_true : (int -> unit) -> t -> unit
(** [iteri_true f v] applies function [f] in turn to all indexes of the elements
    of [v] which are set (i.e. [true]); indexes are visited from least
    significant to most significant. *)

val gray_iter : (t -> unit) -> int -> unit
(** [gray_iter f n] iterates function [f] on all bit vectors of length [n], once
    each, using a Gray code. The order in which bit vectors are processed is
    unspecified. *)
...
val bw_and : t -> t -> t
(** bitwise AND; raises [Invalid_argument] if the two vectors do not have the
    same length *)

val bw_or : t -> t -> t
(** bitwise OR; raises [Invalid_argument] if the two vectors do not have the
    same length *)

val bw_xor : t -> t -> t
(** bitwise XOR; raises [Invalid_argument] if the two vectors do not have the
    same length *)
...
(** {3 Bitwise in place operations.}
    ...
    For example [bw_and_in_place ~dst:a a b] will store in [a] the result of the
    operation [bw_and a b]. *)

val bw_and_in_place : dst:t -> t -> t -> unit
(** bitwise AND in place into [dst]; raises [Invalid_argument] if the three
    vectors do not have the same length *)

val bw_or_in_place : dst:t -> t -> t -> unit
(** bitwise OR in place into [dst]; raises [Invalid_argument] if the three
    vectors do not have the same length *)

val bw_xor_in_place : dst:t -> t -> t -> unit
(** bitwise XOR in place into [dst]; raises [Invalid_argument] if the three
    vectors do not have the same length *)

val bw_not_in_place : dst:t -> t -> unit
(** bitwise NOT in place into [dst]; raises [Invalid_argument] if the two
    vectors do not have the same length *)
...
(** {2 Input/output in a machine-independent format}
    ...
    and independent of the OCaml version. For a bit vector of length [n], the
    number of bytes of this external representation is 8+ceil(n/8). *)
...
(** {2 Interpretation of bit vectors as integers}
    ...
    [to_xxx] functions truncate when the bit vector is too wide. Suffix [_s]
    means that sign bit is kept, and [_us] that it is discarded. *)
...
val of_z : int -> Z.t -> t
(** Only the absolute value, size of the output bitvector *)
