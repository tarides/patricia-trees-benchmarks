...
struct
  ...
  type status =
    | Invisible
        (** The vertex have already been added into the partition and is hidden
            until the end of the search. *)
    | Parent of level
        (** The vertex have been visited and given a [level]. For the algorithm,
            this implies that there is a path between this vertex and the
            current vertex. *)
  ...
  type loop =
    ...
    | Loop of N.t * level
        (** The vertex is inside at least one loop, and level is the smallest
            level of all these loops *)
  ...
  type state = {
    ...
    succs : N.t -> N.t Std.Sequence.t; (* Successors transition. *)
    stack : N.t Stack.t;
  }

  (** Visit [vertex], and all the vertices reachable from [vertex] which have
      not been explored yet (this is a depth-first search). Also gives
      [partition], which is the partition built so far
