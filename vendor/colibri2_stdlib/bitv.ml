...
module Bytes = struct
  ...
  let hash_fold_t state t =
    Base.String.hash_fold_t state (Bytes.unsafe_to_string t)
end

type t = { length : Base.int; bits : Bytes.t } [@@deriving eq, ord, hash]
(* invariant: the unused high bits of the last byte are zeros (if any) *)

let pp fmt t =
  for i = 0 to Bytes.length t.bits - 1 do
    Fmt.pf fmt "%xi" (byte t.bits i)
  done
...
let fill v ofs len b =
  ...
  if len > 0 then (
    ...
    for i = start to start + n - 1 do
      Bytes.unsafe_set v.bits i x
    done;
    (* incomplete last byte, if any *)
    let s = (len - first) land 7 in
    if s > 0 then
      let stop = ofs + len in
      for i = stop - s to stop - 1 do
        unsafe_set v i b
      done)
...
let pop v =
  ...
  for i = 0 to q - 1 do
    let i = i lsl 3 in
    r := !r + (Int64.popcount @@ Bytes.get_int64_ne b i)
  done;
  for i = q' to n - 1 do
    r := !r + Int.popcount (byte b i)
  done;
  !r

let pop_and v w =
  if w.length <> v.length then invalid_arg "Bitv.pop_and";
  let n = Bytes.length v.bits in
  let q = n lsr 3 in
  let q' = q lsl 3 in
  let r = ref 0 in
  ...
  while !i < q' do
    r :=
      ...
      + (Base.Int64.popcount
        @@ Base.Int64.( land )
             (Base.Bytes.unsafe_get_int64 v.bits !i)
             (Base.Bytes.unsafe_get_int64 w.bits !i));
    i := !i + 8
  done;
  for i = q' to n - 1 do
    r := !r + Base.Int.popcount (byte v.bits i land byte w.bits i)
  done;
  !r
...
let xor_and v w =
  ...
  while !i < q' do
    r :=
      Base.Int64.( lxor ) !r
        (Base.Int64.( land )
           (Base.Bytes.unsafe_get_int64 v !i)
           (Base.Bytes.unsafe_get_int64 w !i));
    i := !i + 8
  done;
  for i = q' to n - 1 do
    r := Base.Int64.( lxor ) !r (Base.Int64.of_int (byte v i land byte w i))
  done;
  let r = !r in
  let open Base.Int64 in
  let r = (r lsr 32) lxor r in
  let r = (r lsr 16) lxor r in
  let r = (r lsr 8) lxor r in
  let r = (r lsr 4) lxor r in
  let r = (r lsr 2) lxor r in
  let r = (r lsr 1) lxor r in
  r land 1L = 1L
...
let[@inline always] bw_and_in_place_internal ~dst ~n b1 b2 =
  ...
  for i = 0 to q - 1 do
    let i = i lsl 3 in
    Bytes.set_int64_ne dst i
      (Int64.( land ) (Bytes.get_int64_ne b1 i) (Bytes.get_int64_ne b2 i))
  done;
  for i = q' to n - 1 do
...
let all_ones v =
  ...
  let rec test i =
    ...
    else byte b i = 0xFF && test (succ i)
  in
  n = 0 || test 0
...
let rec rotatel v d =
  if d < 0 then rotater v (-d)
  else
    let n = v.length in
    if d = 0 || n = 0 then copy v
    else
      let d = d mod n in
      let r = create n false in
      unsafe_blit v.bits 0 r.bits d (n - d);
      (* shiftl *)
      unsafe_blit v.bits (n - d) r.bits 0 d;
      (* wraparound ms to ls *)
      r

and rotater v d =
  if d < 0 then rotatel v (-d)
  else
    let n = v.length in
    if d = 0 || n = 0 then copy v
    else
      ...
      (* shiftr *)
      unsafe_blit v.bits 0 r.bits (n - d) d;
      (* wraparound ls to ms *)
      r
...
struct
  ...
  let of_string s =
    ...
    for i = 0 to n - 1 do
      ...
      else if not (Char.equal c '0') then invalid_arg "Bitv.of_string"
    done;
    v
end
...
let of_int_gen len getbyte getbit =
  let v = create len false in
  for i = 0 to (len lsr 3) - 1 do
    set_byte v.bits i (getbyte i)
  done;
  for i = len land lnot 7 to len - 1 do
    unsafe_set v i (getbit i)
  done;
  v

let getbyte x i = (x lsr (8 * i)) land 0xFF
let getbit x i = (x lsr i) land 1 > 0
let of_int_us x = of_int_gen (Sys.int_size - 1) (getbyte x) (getbit x)
let of_int_s x = of_int_gen Sys.int_size (getbyte x) (getbit x)
...
let getbit32 x i = Int32.(Int32.logand (Int32.shift_right x i) 1l > 0l)
let of_int32_us x = of_int_gen 31 (getbyte32 x) (getbit32 x)
let of_int32_s x = of_int_gen 32 (getbyte32 x) (getbit32 x)
...
let getbit64 x i = Int64.(Int64.logand (Int64.shift_right x i) 1L > 0L)
let of_int64_us x = of_int_gen 63 (getbyte64 x) (getbit64 x)
let of_int64_s x = of_int_gen 64 (getbyte64 x) (getbit64 x)
...
let to_z v = Z.of_bits @@ Bytes.unsafe_to_string v.bits

let of_z n v =
  let b = create n false in
  let c = Bytes.unsafe_of_string @@ Z.to_bits v in
  let len = min n (Bytes.length c lsl 3) in
  unsafe_blit c 0 b.bits 0 len;
  b
