...
(*   (1* let debug ~level op_name p = *1) *)
(*   (1*   debug ~dkey ~level "%s %S (project %s)" *1) *)
(*   (1*     op_name *1) *)
(*   (1*     !internal_name *1) *)
(*   (1*     (Project.get_debug_name p) *1) *)

  module Datatype = D
  module Info = Info
(*   (1* module Tbl = Hashtbl.Make(Project) *1) *)
(*   include Info *)

(*   type t = { mutable state: Local_state.t; mutable computed: bool } *)

(*   (1* Project --> plugin state. *1) *)
...
(*   let clean () = *)
(*     (1*      Format.printf "cleaning %s@." !internal_name;*) 
...
(*     fun p -> *)
(*       assert (not (mem p)); *)
(*       (1* For efficiency purpose, do not create the initial project twice: *)
(*          directly get it *1) *)
(*       let mk () = *)
(*         if !first then begin *)
(*           first := false; *)
...
(*   (1* ******* TOUCH THE FOLLOWING AT YOUR OWN RISK: DANGEROUS CODE ******** *1) *)
...
(*         end else *)
(*           (1* invariant: the found state is equal to the default one since it *)
(*              has been just created. *)
(*              Do not call Local_state.create to don't break sharing *1) *)
...
(*     in *)
(*     State.create *)
(*       (1* we will marshal the value [()] if the state is unmarshable *1) *)
...
(*     (1* register this state in the static graph and in projects *1) *)
...
struct
  ...
  (*         (1*       Format.printf "%S: %S %S@." Info.name H.Key.name Data.name;*) 
  (*         let x = *)
  (*           if D.mem_project == Datatype.never_any_project then *)
  (*             false *)
  (*           else *)
  (*             (1* [TODO] BUG: if [Data.mem_project f v] returns [true] and there are *)
...
struct
  ...
  (*       let set x = state := x *)
  (*       let clear_some_projects f h = *)
  (*         (1* if Data.mem_project == Datatype.never_any_project then *1) *)
  (*         (1*   false *1) *)
  (*         (1* else *1) *)
  (*           let found = *)
  (*             W.fold *)
  (*               (fun k l ->  (1* if Data.mem_project f k then k :: l else *1) l) h [] *)
  (*           in *)
...
module Hashconsing_tbl = Hashconsing_tbl_not_weak
...
(*             false *)
(*           else *)
(*             (1* cannot remove a single element from a queue *1) *)
(*             try *)
