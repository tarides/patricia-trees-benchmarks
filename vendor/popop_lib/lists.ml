...
let fold_product f acc l1 l2 =
  List.fold_left
    (fun acc e1 -> List.fold_left (fun acc e2 -> f acc e1 e2) acc l2)
    acc l1
...
let product l1 l2 = fold_product (fun acc a b -> (a, b) :: acc) [] l1 l2
let flatten_rev fl = List.fold_left (fun acc l -> List.rev_append l acc) [] fl
...
let part cmp l =
  ...
  | e :: l ->
      let rec aux acc curr last = function
        ...
        | a :: l -> aux ((last :: curr) :: acc) [] a l
      in
      aux [] [] e l

let rec first f = function
  | [] -> raise Not_found
  | a :: l -> ( match f a with None -> first f l | Some r -> r)

let find_nth p l =
  let rec aux p n = function
    | [] -> raise Not_found
    | a :: l -> if p a then n else aux p (n + 1) l
  in
  aux p 0 l
...
let first_nth f l =
  let rec aux f n = function
    | [] -> raise Not_found
    | a :: l -> ( match f a with None -> aux f (n + 1) l | Some r -> (n, r))
  in
  aux f 0 l
