...
module Make (X : Ordered) = struct
  ...
  let rec merge h1 h2 =
    ...
    | T (_, x, xprio, a1, b1), T (_, y, yprio, a2, b2) ->
        if X.le x xprio y yprio then make x xprio a1 (merge b1 h2)
        else make y yprio a2 (merge h1 b2)
  ...
  let rec reprio db = function
    | E -> E
    | T (_, x, xprio0, a0, b0) as old -> (
        let a = reprio db a0 in
        let b = reprio db b0 in
        let xprio = X.reprio db x in
        if a == a0 && b == b0 && X.equal_prio xprio0 xprio then old
        else
          match (a, b) with
          | E, E -> T (1, x, xprio, a, b)
          | ((T (_, ax, aprio, _, _) as h), E | E, (T (_, ax, aprio, _, _) as h))
            when X.le x xprio ax aprio ->
              make x xprio E h
  ...
  let rec fold f acc = function
    | E -> acc
    | T (_, x, p, t1, t2) -> fold f (fold f (f acc x p) t1) t2
end
