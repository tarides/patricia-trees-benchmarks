...
let add x = function Empty -> Elt x | t -> Add (x, t)
let append t x = match t with Empty -> Elt x | t -> App (t, x)
let list = function [] -> Empty | [ x ] -> Elt x | xs -> List xs
...
let rec ulist = function [] -> Empty | x :: xs -> concat x (ulist xs)
...
let rec filter f = function
  ...
  | App (ts, x) ->
      let ts = filter f ts in
      if f x then append ts x else ts
  | List xs -> list (List.filter f xs)
  | Concat (a, b) -> concat (filter f a) (filter f b)

let rec filter_homo f = function
  | Empty -> Empty
  | Elt x as e -> if f x then e else Empty
  | Add (x, ts) as e ->
      if f x then
        let ts' = filter_homo f ts in
        if ts' == ts then e else add x ts'
      else filter_homo f ts
  | App (ts, x) as e ->
      let ts' = filter_homo f ts in
      if f x then if ts' == ts then e else append ts' x else ts'
  | List l as e ->
      let rec aux f = function
        | [] -> []
        | x :: l as e ->
            let l' = aux f l in
            if f x then if l == l' then e else x :: l' else l'
      in
      let l' = aux f l in
      if l' == l then e else List l'
  | Concat (a, b) as e ->
      let a' = filter_homo f a in
      let b' = filter_homo f b in
      if a == a' && b == b' then e else concat a' b'

let rec filter_map_homo f = function
  | Empty -> Empty
  | Elt x as e -> (
      match f x with Some x' -> if x == x' then e else Elt x' | None -> Empty)
  | Add (x, ts) as e -> (
      match f x with
      | Some x' when x == x' ->
...
let is_num_elt n m =
  try fold_left (fun n _ -> if n < 0 then raise Exit else n - 1) n m = 0
  with Exit -> false
...
(** t' is inside t *)
let rec unadd_from t' t =
  if t' == t then []
  else
    match t with
    | Add (x, t'') -> x :: unadd_from t' t''
    | _ -> invalid_arg "Bag.unadd_from"
