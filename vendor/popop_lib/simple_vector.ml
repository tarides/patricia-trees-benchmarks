...
let dumb : 'a. 'a =
  ...
  Sys.opaque_identity (Obj.magic dumb_addr)

let create size = { data = Array.make size dumb; size }
let size t = t.size
...
let init_inc_size new_size f t =
  if t.size < new_size then
    ...
    if old_capacity < new_size then (
      let init i =
        if i < t.size then t.data.(i) else if i < new_size then f i else dumb
      in
