...
let channel_contents_fmt cin fmt =
  ...
  do
    Format.pp_print_string fmt
      (if !n = 1024 then Bytes.unsafe_to_string buff
       else Bytes.unsafe_to_string (Bytes.sub buff 0 !n))
  done

let channel_contents_buf cin =
  let buf = Buffer.create 1024 and buff = Bytes.make 1024 ' ' in
  let n = ref 0 in
  while
    n := input cin buff 0 1024;
    !n <> 0
...
let rec fold_channel f acc cin =
  try fold_channel f (f acc (input_line cin)) cin with End_of_file -> acc
...
let file_contents_fmt f fmt =
  try
    ...
    close_in cin
  with _ -> invalid_arg (Printf.sprintf "(cannot open %s)" f)
...
*)
    ...
    if d = Filename.current_dir_name then
      ...
      let b = Filename.basename f in
      if f = b then b :: acc else aux (b :: acc) d
  in
  aux [] f
...
let absolutize_filename dirname f =
  if Filename.is_relative f then Filename.concat dirname f else f
...
let uniquify file =
  ...
  let i = try String.rindex file '.' with _ -> String.length file in
  let name = String.sub file 0 i in
  let ext = String.sub file i (String.length file - i) in
  let i = ref 1 in
  while Sys.file_exists (name ^ "_" ^ string_of_int !i ^ ext) do
    incr i
  done;
