...
(** Functor building an implementation of the set structure given a totally
    ordered type. *)
module MakeOfMap (M : Map_intf.MapUnit) :
  S with type 'a M.t = 'a M.t and type M.key = M.key

(** Functor building an implementation of the set structure given a totally
    ordered type. *)
module Make (Ord : Map_intf.OrderedType) : S with type M.key = Ord.t

(** Functor building an implementation of the hasconsed set structure given a
    totally ordered hashconsed map. *)
