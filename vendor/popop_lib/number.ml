...
type constant = ConstInt of integer_constant | ConstReal of real_constant
...
let power2 n = string_of_big_int (power_int_positive_int 2 n)
...
type dec_real_format = PrintDecReal of part_real_format * real_format
...
let print_dec_int support fmt i =
  let fallback i = force_support support.def_int_support (fprintf fmt) i in
  if
    (not support.long_int_support)
    && compare_big_int (big_int_of_string i) simplify_max_int > 0
  then fallback i
  else
    check_support support.dec_int_support (Some "%s") (fprintf fmt) fallback i

let print_hex_int support fmt =
  check_support support.hex_int_support (Some "0x%s")
    (fun s i ->
      assert support.long_int_support;
      fprintf fmt s i)
  ...
  check_support support.dec_real_support
    ...
    (check_support support.frac_real_support None
       (fun (PrintFracReal (exp_zero, exp_pos, exp_neg)) i f e ->
         ...
         if e = 0 then fprintf fmt exp_zero (i ^ f)
         else if e > 0 then fprintf fmt exp_pos (i ^ f) ("1" ^ String.make e '0')
         else fprintf fmt exp_neg (i ^ f) ("1" ^ String.make (-e) '0'))
       (force_support support.def_real_support (fun def i f e ->
            fprintf fmt def
              (sprintf "%s_%s_e%s" i f
                 (match e with None -> "0" | Some e -> remove_minus e)))))

let print_hex_real support fmt =
  check_support support.hex_real_support (Some "0x%s.%sp%s")
    ...
    (check_support support.frac_real_support None
       (fun (PrintFracReal (exp_zero, exp_pos, exp_neg)) i f e ->
         ...
         if e = 0 then fprintf fmt exp_zero dec
         else if e > 0 then fprintf fmt exp_pos dec (power2 e)
         else fprintf fmt exp_neg dec (power2 (-e)))
       (force_support support.def_real_support (fun def i f e ->
            fprintf fmt def
              (sprintf "0x%s_%s_p%s" i f
                 (match e with None -> "0" | Some e -> remove_minus e)))))
