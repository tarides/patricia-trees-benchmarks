...
module type S = sig
  include Map_intf.PMap

  type 'a view = Empty | Node of 'a view * key * 'a * 'a view * int
...
module Make (Ord : Map_intf.OrderedType) = struct
  ...
  type 'a view = 'a t = Empty | Node of 'a view * key * 'a * 'a view * int
  ...
  let pp pp =
    if pp == Fmt.nop || pp == Pp.nothing then
      Pp.iter2 iter Pp.comma Fmt.nop Ord.pp Fmt.nop
    else Pp.iter2 iter Pp.comma Pp.arrow Ord.pp pp
  ...
  let rec change f x = function
    ...
    | Node (l, v, d, r, h) ->
        ...
        else if c < 0 then bal (change f x l) v d r
        else bal l v d (change f x r)

  let rec add_change empty add x b = function
    | Empty -> Node (Empty, x, empty b, Empty, 1)
    | Node (l, v, d, r, h) ->
        let c = Ord.compare x v in
        if c = 0 then Node (l, x, add b d, r, h)
        else if c < 0 then bal (add_change empty add x b l) v d r
        else bal l v d (add_change empty add x b r)
    ...
    | Node (l1, v1, d1, r1, h1), Node (l2, v2, d2, r2, h2) -> (
        if h1 >= h2 then
          if h2 = 1 then
            ...
            | Some d2 ->
                concat_or_join (union f l1 l2) v1 (f v1 d1 d2) (union f r1 r2)
        else if h1 = 1 then
          ...
          | Some d1 ->
              concat_or_join (union f l1 l2) v2 (f v2 d1 d2) (union f r1 r2))
  ...
  let rec submap pr s1 s2 =
    ...
    | Node (l1, v1, d1, r1, _), (Node (l2, v2, d2, r2, _) as t2) ->
        let c = Ord.compare v1 v2 in
        if c = 0 then pr v1 d1 d2 && submap pr l1 l2 && submap pr r1 r2
        else if c < 0 then
          submap pr (Node (l1, v1, d1, Empty, 0)) l2 && submap pr r1 t2
        else submap pr (Node (Empty, v1, d1, r1, 0)) r2 && submap pr l1 t2
  ...
  let rec disjoint pr s1 s2 =
    ...
    | Node (l1, v1, d1, r1, _), (Node (l2, v2, d2, r2, _) as t2) ->
        let c = Ord.compare v1 v2 in
        if c = 0 then pr v1 d1 d2 && disjoint pr l1 l2 && disjoint pr r1 r2
        else if c < 0 then
          disjoint pr (Node (l1, v1, d1, Empty, 0)) l2 && disjoint pr r1 t2
        else disjoint pr (Node (Empty, v1, d1, r1, 0)) r2 && disjoint pr l1 t2
  ...
  let rec find_def def x = function
    ...
    | Node (l, v, d, r, _) ->
        let c = Ord.compare x v in
        if c = 0 then d else find_def def x (if c < 0 then l else r)
  ...
  let rec find_exn exn x = function
    ...
    | Node (l, v, d, r, _) ->
        let c = Ord.compare x v in
        if c = 0 then d else find_exn exn x (if c < 0 then l else r)

  let rec find_remove x = function
    | Empty -> (Empty, None)
    | Node (l, v, d, r, _h) ->
        let c = Ord.compare x v in
        if c = 0 then (merge_bal l r, Some d)
        else if c < 0 then
  ...
  let rec find_smaller_opt cand x = function
    ...
    | Node (l, v, d, r, _) ->
        ...
        else if c < 0 then find_smaller_opt cand x l
        else find_smaller_opt (Some (x, d)) x r
  ...
  let fold2_inter f m1 m2 acc =
    let rec aux acc e1_0 e2_0 =
      ...
      | More (v1, d1, r1, e1), More (v2, d2, r2, e2) ->
          let c = Ord.compare v1 v2 in
          if c = 0 then aux (f v1 d1 d2 acc) (cons_enum r1 e1) (cons_enum r2 e2)
          else if c < 0 then aux acc (cons_enum r1 e1) e2_0
          else aux acc e1_0 (cons_enum r2 e2)
    in
    aux acc (cons_enum m1 End) (cons_enum m2 End)
  ...
  let translate f m =
    let rec aux last = function
      ...
      | Node (l, v, d, r, h) ->
          ...
          | None -> ()
          | Some last ->
              if Ord.compare last v >= 0 then
                invalid_arg "Map.translate : given function incorrect");
          let r, last = aux (Some v) r in
          (Node (l, v, d, r, h), last)
    in
    let m, _ = aux None m in
    m
  ...
  let add_new e x v m =
    change (function Some _ -> raise e | None -> Some v) x m

  let is_num_elt n m =
    try fold (fun _ _ n -> if n < 0 then raise Exit else n - 1) m n = 0
    with Exit -> false
  ...
  let val_enum = function End -> None | More (v, d, _, _) -> Some (v, d)
  let next_enum = function End -> End | More (_, _, r, e) -> cons_enum r e
  ...
  let rec fold_left f accu m =
    match m with
    | Empty -> accu
    | Node (l, v, d, r, _) -> fold_left f (f (fold_left f accu l) v d) r

  let rec fold_decr f accu m =
    match m with
    | Empty -> accu
    | Node (l, v, d, r, _) -> fold_decr f (f (fold_decr f accu r) v d) l

  let of_list l = List.fold_left (fun acc (k, d) -> add k d acc) empty l
  let add_opt x o m = match o with None -> remove x m | Some y -> add x y m
  ...
  let hash_fold_t h s t = fold (fun k v s -> Ord.hash_fold_t (h s v) k) t s
end
