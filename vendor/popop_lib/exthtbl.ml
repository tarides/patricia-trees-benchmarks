...
module type HashedType = sig
  ...
  (** A hashing function on keys. It must be such that if two keys are equal
      ...
      include
      - ([(=)], {!Hashtbl.hash}) for comparing objects by structure (provided
        objects do not contain floats)
      - ([(fun x y -> compare x y = 0)], {!Hashtbl.hash}) for comparing objects
        by structure and handling {!Stdlib.nan} correctly
      - ([(==)], {!Hashtbl.hash}) for comparing objects by physical equality
        (e.g. for mutable or cyclic objects). *)
end
...
module type S = sig
  ...
  val add_change : ('b -> 'a) -> ('a -> 'b -> 'a) -> 'a t -> key -> 'b -> unit
...
module type Hashtbl = sig
  ...
  module Make (X : HashedType) : S with type key = X.t
end
...
module Hashtbl = struct
  ...
  module MakeSeeded (H : SeededHashedType) = struct
    ...
    and 'b bucketlist = Empty | Cons of key * 'b * 'b bucketlist
    ...
    let rec find_rec exn key = function
      | Empty -> raise exn
      | Cons (k, d, rest) -> if H.equal key k then d else find_rec exn key rest
    ...
    let find_all h key =
      let rec find_in_bucket = function
        | Empty -> []
        | Cons (k, d, rest) ->
            if H.equal k key then d :: find_in_bucket rest
    ...
    let replace h key info =
      let rec replace_bucket = function
        ...
        | Cons (k, i, next) ->
            if H.equal k key then
              if i == info then raise Exit else Cons (key, info, next)
            else Cons (k, i, replace_bucket next)
      in
      let i = key_index h key in
      let l = h.data.(i) in
      try h.data.(i) <- replace_bucket l with
      | Not_found ->
    ...
    let add_change add change h key value =
      let rec replace_bucket = function
        | Empty -> raise Not_found
        | Cons (k, i, next) ->
            if H.equal k key then
              let info = change i value in
              if i == info then raise Exit else Cons (key, info, next)
            else Cons (k, i, replace_bucket next)
      in
      let i = key_index h key in
      ...
      | Exit -> ()
      | Not_found ->
          let info = add value in
          h.data.(i) <- Cons (key, info, l);
          h.size <- h.size + 1;
          if h.size > Array.length h.data lsl 1 then resize key_index h

    let add_new e h x v =
      change (function Some _ -> raise e | None -> Some v) h x

    let mem h key =
      let rec mem_in_bucket = function
        | Empty -> false
        | Cons (k, _, rest) -> H.equal k key || mem_in_bucket rest
      in
    ...
    let for_all f h =
      let rec do_bucket f b =
        match b with
        | Empty -> true
        | Cons (k, d, rest) -> f k d && do_bucket f rest
      in
      let rec aux f i d len =
        len <= i || (do_bucket f d.(i) && aux f (i + 1) d len)
      in
      aux f 0 h.data (Array.length h.data)
    ...
    let mapi f h =
      let rec do_bucket f b =
        match b with
        | Empty -> Empty
        | Cons (k, d, rest) -> Cons (k, f k d, do_bucket f rest)
      in
      let d = h.data in
      for i = 0 to Array.length d - 1 do
        d.(i) <- do_bucket f d.(i)
      ...
      let rec do_bucket f b =
        ...
        | Cons (k, d, rest) -> (
            ...
            | None ->
                h.size <- h.size - 1;
                do_bucket f rest
            | Some v -> Cons (k, v, do_bucket f rest))
      in
      let d = h.data in
      for i = 0 to Array.length d - 1 do
        d.(i) <- do_bucket f d.(i)
    ...
    let filter f h =
      let rec do_bucket f b =
        ...
        | Cons (k, d, rest) ->
            if f k d then
              let rest' = do_bucket f rest in
              if rest' == rest then b else Cons (k, d, rest')
            else (
              h.size <- h.size - 1;
              do_bucket f rest)
      in
    ...
    let stats h =
      let mbl =
        Array.fold_left (fun m b -> max m (bucket_length 0 b)) 0 h.data
      in
    ...
    let find_def h d k = try find h k with Not_found -> d

    let rec find_opt_rec key = function
      | Empty -> None
      | Cons (k, d, rest) ->
          if H.equal key k then Some d else find_opt_rec key rest
          ...
          if H.equal key k1 then Some d1
            ...
            | Cons (k2, d2, rest2) -> (
                if H.equal key k2 then Some d2
                  ...
                  | Empty -> None
                  | Cons (k3, d3, rest3) ->
                      if H.equal key k3 then Some d3 else find_opt_rec key rest3
                ))
    ...
    let pp pp_elt =
      Fmt.iter_bindings iter ~sep:Fmt.semi
        (Fmt.pair ~sep:(Fmt.any "⇒") H.pp pp_elt)
  end
