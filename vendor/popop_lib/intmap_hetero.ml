...
module type S1 = sig
  ...
  val pp : unit Pp.pp -> 'b pp -> 'b t Pp.pp
end
...
struct
  ...
  let open_t (type a) (type b) (t : b t) (_ : a key) : (a, b) D.t DInt.M.t =
    Obj.magic t
  ...
  let union f t1 t2 =
    DInt.M.union
      (fun i d1 d2 ->
        f.union
          (Obj.magic (i : int) :> exi K.t)
          (* at some time this key have been
                                            given *)
          d1 d2)
      t1 t2
  ...
  let mapi f t =
    DInt.M.mapi
      (fun i d ->
        f.mapi (Obj.magic (i : int) :> exi K.t) (* same thing than for iter *) d)
      t
...
struct
  ...
  let pp sep1 sep2 printkey pp fmt (t : 'b t) =
    let printkey fmt i =
      printkey.printk fmt (Obj.magic (i : int) :> exi K.t)
      (* same thing than for iteri *)
    in
    Pp.iter2 DInt.M.iter sep1 sep2 printkey pp fmt t
end
