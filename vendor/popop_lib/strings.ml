...
let ends_with s suf =
  let rec aux s suf suflen offset i =
    i >= suflen || (s.[i + offset] = suf.[i] && aux s suf suflen offset (i + 1))
  in
...
let pad_right c s i =
  ...
  else if sl > i then String.sub s 0 i
  else s
...
module Make (_ : sig end) = struct
  ...
  let pp fmt i =
    try Format.pp_print_string fmt (view i)
...
let find_new_name used_names s =
  let rec aux used_names s n =
    ...
    if DStr.H.mem used_names sid then aux used_names s (n + 1) else (sid, n + 1)
  in
  try
